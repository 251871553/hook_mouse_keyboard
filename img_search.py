...
#待搜索的图片列表
img_list=glob.glob(r'dataset/*.png')
...
class calculation(public):
      def __init__(self):
          public.__init__(self)
# 最简单的以灰度直方图作为相似比较的实现  
      def classify_gray_hist(self,image1,image2,size = (256,256)):  
    # 先计算直方图  
    # 几个参数必须用方括号括起来  
...
class call_result(calculation):
       def __init__(self):
          calculation.__init__(self)
       def accurate(self,img1):
           for  i  in img_list:
              img2 = cv2.imread(i)
              degree = self.classify_pHash(img1,img2)  
              if degree==1:
                 print i,degree,'***绝对匹配***'
              else:
                 if  degree  < 10:   #越接近0越相似
                     print  i,degree
       def vague(self,img1):
           for  i  in  img_list:
              img2 = cv2.imread(i)
...
result=call_result()
img1 = cv2.imread('dataset/100100.png')
result.vague(img1) 
#result.accurate(img1) 
